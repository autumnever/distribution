...
public class Hw2Part1 {
  ...
  public static class TokenizerMapper 
    ...
    public void map(Object key, Text value, Context context
      ...
      while (itr.hasMoreTokens()) {
        String tmpkey = (String)itr.nextToken() + " " + (String)itr.nextToken();
        //double time = Double.valueOf(itr.nextToken());
        String tmpvalue = "1" + " " + (String)itr.nextToken();
        word.set(tmpkey);
        mapoutvalue.set(tmpvalue);
        context.write(word, mapoutvalue);  //source destination 1 time
      }*/
  
      //BufferedReader in = new BufferedReader(new InputStreamReader(value.toString()));
      //String s;
      StringTokenizer itr = new StringTokenizer(value.toString(),"\n");
      while (itr.hasMoreTokens()) {
        String[] str=itr.nextToken().split(" ");  //every line segmentation with " "
        if(str.length == 3 && Float.parseFloat(str[2])>=0){
  ...
  public static class IntSumCombiner
    ...
    public void reduce(Text key, Iterable<Text> values,
      ...
      for (Text val : values) {
        ...
        sumtime += Float.valueOf(String.valueOf(val).split(" ")[1]);
      }
  ...
  public static class IntSumReducer
    ...
    public void reduce(Text key, Iterable<Text> values, 
      ...
      float tmp_ave_time = sumtime/count;
